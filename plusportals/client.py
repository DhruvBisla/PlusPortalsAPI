...
class Client():
    ...
    def __init__(self, **kwargs):
        ...
        (Client.setCredentials(**kwargs), None)[Client.hasSetCredentials]
    ...
    def login(self) -> None:
        ...
        specHeaders = {
            'cookie': '__cfduid={}; ppschoollink={}; UGUID={}; __RequestVerificationToken={}; _pps=-480'.format(self.session.cookies.get_dict().get("__cfduid"), info.SCHOOL_NAME, self.session.cookies.get_dict().get("UGUID"), self.session.cookies.get_dict().get("__RequestVerificationToken"))
        }
    ...
    def getDetails(self):
        ...
        specHeaders = {
            'cookie': '__cfduid={}; ppschoollink=QuarryLaneSchool; __RequestVerificationToken={}; _pps=-480; ASP.NET_SessionId={}; emailoption=RecentEmails; UGUID={}; ppusername={}; .ASPXAUTH={}'.format(session.cookies.get_dict().get('__cfduid'), session.cookies.get_dict().get('__RequestVerificationToken'), session.cookies.get_dict().get('ASP.NET_SessionId'), session.cookies.get_dict().get('UGUID'), session.cookies.get_dict().get('ppusername'), session.cookies.get_dict().get('.ASPXAUTH'))
        }
